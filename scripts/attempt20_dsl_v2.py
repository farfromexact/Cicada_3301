...
from lp_lab.r015_dsl_v2 import (DIVINITY, STATE_OPERATORS, apply_symbol_permutation,
                             build_language_model, enumerate_recipes, encrypt_recipe,
                             primes)


SPEC_PATH = ROOT / "hypotheses/H029-finite-dsl-v2.json"
...
WORKER_PATH = ROOT / "scripts/attempt20_dsl_v2_worker.py"
...
def make_gate(programs: list[tuple[str, ...]], model: dict) -> tuple[list[dict], dict]:
    ...
    for i in range(99):
        ...
        for page_index in range(3):
            ...
            start = (i * 31 + page_index * 43 + 17) % (len(source) - length)
    ...
    positive_scores = [sum(page["natural_score"] for page in row["pages"]) / 3.0
...
def validate_gate(output: dict, gate_private: dict, programs: list[tuple[str, ...]], model: dict) -> dict:
    ...
    for job_id, private in gate_private["private"].items():
        ...
        independent_pages = [dict(values=page["cipher"]) for page in private["pages"]]
        independent = independent_rank(independent_pages, programs, model)
...
def main() -> int:
    ...
    record = dict(schema=1, hypothesis=HYPOTHESIS, attempt="auto-cycle R015-A v2",
    ...
    try:
        ...
        statistics = {"status": "negative", "verification_status": "passed",
                      ...
                      "negative_scope": "Only H029-v2's 49 source-backed grammar programs, one state atom, page-local state-zero contract and two declared control families.",
