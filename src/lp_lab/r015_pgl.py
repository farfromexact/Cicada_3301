...
def segment_points(raw: str, runes: str) -> tuple[list[list[int]], dict]:
    """Map only literal '-' to infinity; every other non-rune is a boundary.

    Slash and whitespace remain represented in the metadata counters and do
    not become projective points.  ASCII grid/hash literals also remain hard
    boundaries and are never silently converted to rune values.
    """
