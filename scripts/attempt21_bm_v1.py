...
from lp_lab.runes import GP, RUNES


SPEC_PATH = ROOT / "hypotheses/H030-berlekamp-massey-v1.json"
CORPUS_PATH = ROOT / "data/attempt1-corpus-v1.json"
WORKER_PATH = ROOT / "scripts/attempt21_bm_worker.py"
...
def independent_metrics(sequence: list[int]) -> dict:
    ...
    return dict(status="eligible", length=n, prefix_length=prefix_length,
                ...
                t_p=rate, full_order=full["order"])
...
def compact_metrics(metrics: dict) -> dict:
    return {name: dict(status=row["status"], length=row["length"], order=row["order"],
                       ...
                       full_order=row.get("full_order")) for name, row in metrics.items()}
...
def main() -> int:
    ...
    try:
        ...
        for key, l_row in l_map.items():
            p_row = p_map[key]
            if key[0] in HOLDOUT and l_row["max_t_p"] <= ALPHA and p_row["max_t_p"] <= ALPHA:
