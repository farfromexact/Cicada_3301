...
from lp_lab.r015_dsl_v3 import (DIVINITY, STATE_OPERATORS, apply_symbol_permutation,
                             build_language_model, enumerate_recipes, encrypt_recipe,
                             primes)


SPEC_PATH = ROOT / "hypotheses/H029-finite-dsl-v3.json"
...
WORKER_PATH = ROOT / "scripts/attempt20_dsl_v3_worker.py"
HYPOTHESIS = "H029-finite-dsl-v3"
CONTROL_COUNT = 999
ALPHA = 0.01 / 3.0
TIE_TOLERANCE = 1e-12
WALL_SECONDS = 7200
...
def make_gate(programs: list[tuple[str, ...]], model: dict) -> tuple[list[dict], list[dict], dict]:
    ...
    half = len(source) // 2
    if half <= max(lengths):
        raise ValueError("heldout synthetic source too short for disjoint D/H gate")
    ...
    for i in range(20):
        ...
        split_pages = {"discovery": [], "holdout": []}
        private_pages = {"discovery": [], "holdout": []}
        for split in ("discovery", "holdout"):
            for page_index in range(2):
                length = lengths[(i + page_index + (0 if split == "discovery" else 2)) % len(lengths)]
                available = half - length
                local_start = (i * 53 + page_index * 71 + (0 if split == "discovery" else 17)) % available
                start = local_start + (0 if split == "discovery" else half)
                plain = source[start:start + length]
                cipher = encrypt_recipe(plain, recipe)
                page_id = f"{job_id}-{split[:1]}{page_index}"
                split_pages[split].append(public_page(page_id, cipher))
                private_pages[split].append(dict(page=page_id, start=start, length=length,
                                                 plaintext=plain, cipher=cipher,
                                                 natural_score=natural_score(plain)))
        discovery_jobs.append(dict(id=job_id, pages=split_pages["discovery"]))
        holdout_jobs.append(dict(id=job_id, pages=split_pages["holdout"]))
        private[job_id] = dict(kind="positive", recipe=list(recipe), pages=private_pages)
    rng = random.Random(SEEDS["gate_negative"])
    negative_jobs = []
    for i in range(99):
        job_id = f"negative-{i:02d}"
        kind = ("uniform" if i < 33 else
                "within_permutation" if i < 66 else "global_s29")
        permutation = None
        if kind == "global_s29":
        ...
        for split in ("discovery", "holdout"):
            for page_index in range(2):
                length = lengths[(i + page_index + 2 + (0 if split == "discovery" else 2)) % len(lengths)]
                available = half - length
                local_start = (i * 31 + page_index * 43 + 17 + (0 if split == "discovery" else 19)) % available
                start = local_start + (0 if split == "discovery" else half)
                base = source[start:start + length]
                if kind == "uniform":
                    cipher = [rng.randrange(29) for _ in range(length)]
                elif kind == "within_permutation":
                    cipher = list(base)
                    rng.shuffle(cipher)
                else:
        ...
        discovery_jobs.append(dict(id=job_id, pages=split_pages["discovery"]))
        holdout_jobs.append(dict(id=job_id, pages=split_pages["holdout"]))
        private[job_id] = dict(kind=kind, pages=private_pages)
    positive_scores = [sum(page["natural_score"] for page in row["pages"]["holdout"]) / 2.0
                       for row in private.values() if row["kind"] == "positive"]
    threshold = min(positive_scores) - 0.25
    return discovery_jobs, holdout_jobs, dict(private=private, threshold=threshold,
                                              positive_count=20, negative_count=99)


def validate_gate(discovery_output: dict, holdout_output: dict, gate_private: dict,
                  programs: list[tuple[str, ...]], model: dict) -> dict:
    discovery_by_id = {row["id"]: row for row in discovery_output["results"]}
    holdout_by_id = {row["id"]: row for row in holdout_output["results"]}
    expected = set(gate_private["private"])
    if set(discovery_by_id) != expected or set(holdout_by_id) != expected:
        raise ValueError("R015-A gate coverage mismatch")
    positives = []
    negatives = []
    for job_id, private in gate_private["private"].items():
        discovery_row = discovery_by_id[job_id]
        holdout_row = holdout_by_id[job_id]
        discovery_pages = [dict(values=page["cipher"]) for page in private["pages"]["discovery"]]
        holdout_pages = [dict(values=page["cipher"]) for page in private["pages"]["holdout"]]
        independent_discovery = independent_rank(discovery_pages, programs, model)
        independent_holdout = independent_rank(holdout_pages, programs, model)
        if (discovery_row["selected"]["program"] != independent_discovery["selected"]["program"] or
                not math.isclose(discovery_row["selected"]["score"], independent_discovery["selected"]["score"], rel_tol=1e-11, abs_tol=1e-11) or
                holdout_row["selected"]["program"] != independent_holdout["selected"]["program"] or
                not math.isclose(holdout_row["selected"]["score"], independent_holdout["selected"]["score"], rel_tol=1e-11, abs_tol=1e-11)):
            raise ValueError(f"R015-A gate independent selection mismatch: {job_id}")
        recipe = tuple(discovery_row["selected"]["program"])
        frozen_holdout = next(item for item in holdout_row["rankings"]
                              if tuple(item["program"]) == recipe)
        expected_holdout = next(item for item in independent_holdout["rankings"]
                                if tuple(item["program"]) == recipe)
        if not math.isclose(frozen_holdout["score"], expected_holdout["score"], rel_tol=1e-11, abs_tol=1e-11):
            raise ValueError(f"R015-A gate frozen holdout mismatch: {job_id}")
        if private["kind"] == "positive":
            exact = recipe == tuple(private["recipe"])
            score_ok = frozen_holdout["score"] >= gate_private["threshold"]
            roundtrip = all(encrypt_recipe(page["plaintext"], tuple(private["recipe"])) == page["cipher"]
                            for split in private["pages"].values() for page in split)
            positives.append(dict(id=job_id, expected=private["recipe"], selected=list(recipe),
                                  exact_program=exact, holdout_score=frozen_holdout["score"], score_ok=score_ok,
                                  roundtrip=roundtrip, status="passed" if exact and score_ok and roundtrip else "negative"))
        else:
            accepted = frozen_holdout["score"] >= gate_private["threshold"]
            negatives.append(dict(id=job_id, kind=private["kind"], selected=list(recipe),
                                  holdout_score=frozen_holdout["score"], false_accept=accepted,
...
def main() -> int:
    ...
    record = dict(schema=1, hypothesis=HYPOTHESIS, attempt="auto-cycle R015-A v3",
    ...
    try:
        ...
        gate_discovery_jobs, gate_holdout_jobs, gate_private = make_gate(programs, model)
        frozen = dict(schema=1, frozen_at_utc=dt.datetime.now(dt.timezone.utc).isoformat(),
                      ...
                      model_public_digest=digest(model_public(model)),
                       ...
                       gate_protocol=dict(discovery_pages_per_job=2, holdout_pages_per_job=2,
                                          selection="discovery selection is frozen before holdout scoring",
                                          max_public_runes=80000),
        ...
        gate_discovery_public = dict(schema=1, hypothesis=HYPOTHESIS,
                                     programs=[list(program) for program in programs],
                                     model=model_public(model), jobs=gate_discovery_jobs)
        gate_holdout_public = dict(schema=1, hypothesis=HYPOTHESIS,
                                   programs=[list(program) for program in programs],
                                   model=model_public(model), jobs=gate_holdout_jobs)
        gate_discovery_output = run_worker(run, "gate-discovery", gate_discovery_public, timeout=1200)
        gate_holdout_output = run_worker(run, "gate-holdout", gate_holdout_public, timeout=1200)
        gate = validate_gate(gate_discovery_output, gate_holdout_output, gate_private, programs, model)
        ...
        statistics = {"status": "negative", "verification_status": "passed",
                      ...
                      "negative_scope": "Only H029-v3's 49 source-backed grammar programs, one state atom, page-local state-zero contract and two declared control families.",
        ...
        for family in control_results:
            ...
            family_leads = [dict(page=page["page"], raw_p=raw, holm_p=holm_p, max_t_p=max_p,
                                 score=score, status="lead" if max_p <= ALPHA else "negative")
