...
SPEC_PATH = ROOT / "hypotheses/H031-pgl-full-v2.json"
...
WORKER_PATH = ROOT / "scripts/attempt22_pgl_v2_worker.py"
HYPOTHESIS = "H031-pgl-full-v2"
MODULUS = 29
INFINITY = 29
PGL_ORDER = 24360
CONTROL_COUNT = 999
...
def make_gate(matrices: list[tuple[int, int, int, int]], model: dict) -> tuple[list[dict], dict]:
    ...
    def natural_score(values):
        return score_scalar(independent_table((1, 0, 0, 1)),
                            independent_counts([values]), model)
    ...
    for i in range(20):
        ...
        for page_index in range(3):
            length = lengths[(i + page_index) % len(lengths)]
            start = (i * 47 + page_index * 71 + 11) % (len(source) - length)
            plain = source[start:start + length]
            cipher = [table[value] for value in plain]
            page_id = f"{job_id}-p{page_index}"
            counts = independent_counts([cipher])
            gate_pages.append(dict(page=page_id, counts=counts))
            private_pages.append(dict(page=page_id, start=start, length=length,
                                      plaintext=plain, cipher=cipher, counts=counts,
                                      natural_score=natural_score(plain)))
        positives.append(dict(id=job_id, pages=gate_pages))
        private[job_id] = dict(kind="positive", matrix=list(matrix), inverse=expected_inverse,
                               pages=private_pages)
    ...
    for i in range(99):
        ...
        kind = ("uniform" if i < 33 else
                "within_permutation" if i < 66 else "global_s30")
        permutation = None
        if kind == "global_s30":
        ...
        private_pages = []
        for page_index in range(3):
            length = lengths[(i + page_index + 2) % len(lengths)]
            start = (i * 31 + page_index * 43 + 3) % (len(source) - length)
            plain = source[start:start + length]
            if kind == "uniform":
                ...
                cipher = [permutation[value] for value in plain]
            page_id = f"{job_id}-p{page_index}"
            counts = independent_counts([cipher])
            gate_pages.append(dict(page=page_id, counts=counts))
            private_pages.append(dict(page=page_id, start=start, length=length,
                                      cipher=cipher, counts=counts))
        negatives.append(dict(id=job_id, pages=gate_pages))
        private[job_id] = dict(kind=kind, pages=private_pages)
    baseline = [sum(page["natural_score"] for page in private[f"positive-{i:02d}"]["pages"]) / 3.0
                for i in range(20)]
...
def validate_gate(output: dict, private: dict, matrices: list[tuple[int, int, int, int]],
    ...
    for job_id, expected in private.items():
        ...
        actual = job["selected"]
        scores = all_matrix_scores([page["counts"] for page in expected["pages"]],
                                   pair_weights, start_table)
        selected_index, selected_score, ties = select_matrix(scores, matrices)
        if actual["matrix"] != list(matrices[selected_index]) or not math.isclose(
                actual["score"], selected_score, rel_tol=1e-10, abs_tol=1e-10):
            raise ValueError(f"R015-C gate independent selection mismatch {job_id}")
        if expected["kind"] == "positive":
            ...
            negatives.append(dict(id=job_id, kind=expected["kind"], score=actual["score"],
                                  false_accept=accepted, ties=ties, independent=True,
...
def main() -> int:
    ...
    try:
        ...
        frozen = dict(schema=1, frozen_at_utc=dt.datetime.now(dt.timezone.utc).isoformat(),
                      ...
                      excluded=["LP2/50"], separator_contract="literal '-' -> infinity(29); slash/whitespace are metadata only; other non-runes are hard boundaries",
        ...
        for index, matrix in enumerate(matrices):
            ...
            if index % 137 == 0 or index == selected_index:
        ...
        statistics = dict(status="negative", verification_status="passed", leads=[],
                          ...
                          negative_scope="Only the fixed literal-hyphen infinity contract, slash/whitespace metadata contract, common PGL matrix, scorer, D/H partition and two declared control families.")
        for family in controls:
